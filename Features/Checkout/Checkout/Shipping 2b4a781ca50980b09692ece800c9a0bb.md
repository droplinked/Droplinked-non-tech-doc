...
- **Auto-Loading:** Triggered by address selection (from address book) OR address form completion.
...
- **BAC 1:** Shipping search MUST trigger automatically after address is selected (from address book) OR after Delivery Details form is completed.
...
- **R1. Trigger:** Logic listens for `AddressForm.isValid` = true OR `Address.isSelected` = true.
- **R2. Visualization:**
